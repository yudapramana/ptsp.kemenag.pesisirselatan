  ...
  <div class="col-sm-12">
    ...
    <div class="card card-default">
      ...
      <div class="card-header text-right bg-teal">
        {{ Form::open(['url'=>(empty($id_unit_pengolah)?'data_layanan':'data_layanan/detail/'.Helper::encrypt($id_unit_pengolah)), 'method'=>'post']) }}
        <div class="row">
          <div class="col-sm-8">
            <div class="input-group">
              {{ Form::text('cari','',['class'=>'form-control', 'placeholder'=>'Pencarian']) }}
              {{ Form::select('id_unit_pengolah',Helper::get_unit_pengolah_arr(),$id_unit_pengolah,['class'=>'form-control','placeholder'=>'-- Semua unit pengolah--']) }}
          ...
          <div class="col-sm-4">
            @if(isset($cari))
              <a href="{{ (empty($id_unit_pengolah)?url('data_layanan'):url('data_layanan/detail/'.Helper::encrypt($id_unit_pengolah)) ) }}" class="btn btn-warning btn-sm"><i class="fa fa-refresh"></i> Reset</a>
            @endif
            @if(Helper::cek_level_akses('data_layanan')->t)
            <a href="{{ (empty($id_unit_pengolah)?url('data_layanan/tambah'):url('data_layanan/tambah/'.Helper::encrypt($id_unit_pengolah))) }}" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Tambah</a>
            @endif
          </div>
        </div>
        <!-- end row -->
          ...
          <table class="table table-bordered table-hovered">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{{ $value->output }}</td>
                <td>{{ $value->lama_layanan }} hari</td>
                <td>{{ $value->biaya }}</td>
                <td class="text-center">
                  @if(Helper::cek_level_akses('data_layanan')->d)
                  <a href="{{url('syarat_layanan/detail')}}/{{Helper::encrypt($value->id)}}" class="btn btn-info btn-sm btn-detail"><i class="fa fa-fw fa-eye"></i></a>
                  @endif
                  @if(Helper::cek_level_akses('data_layanan')->u)
                  <a href="{{url('data_layanan/ubah')}}/{{Helper::encrypt($value->id)}}" class="btn btn-warning btn-sm btn-ubah"><i class="fa fa-fw fa-edit"></i></a>
                  @endif
                  @if(Helper::cek_level_akses('data_layanan')->h)
...
@section('js')
  <script type="text/javascript">
    function open_modal(id) {
      $('.modal-message').modal();
      $('#submit_hapus').attr("href","{{url('data_layanan')}}/hapus/"+id);
    }
