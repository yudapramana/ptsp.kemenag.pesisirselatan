...
<div class="container-fluid">
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">Ubah Syarat Layanan</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ url('dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item"><a href="{{ url('syarat_layanan') }}">Syarat Layanan</a></li>
                <li class="breadcrumb-item active">Ubah Syarat Layanan</li>
            </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div><!-- /.container-fluid -->
...
<div class="row">
  <div class="col-sm-12">
    ...
    <div class="card card-default">
      {{ Form::open(['url'=>'syarat_layanan/ubah/'.Helper::encrypt($data->id), 'method'=>'post', 'class'=>'form-horizontal']) }}
      <!-- end card-header -->
      ...
      <div class="card-body">
          @if(empty($id_data_layanan))
            <div class="form-group {{ $errors->has('id_data_layanan') ? 'has-error' : '' }}">
              ...
              <div class="col-sm-12">
                {{ Form::select('id_data_layanan',Helper::get_data_layanan_arr(),$data->id_data_layanan,['class'=>'form-control', 'placeholder'=>'-- Nama layanan --']) }}
                <span class="help-block">{{ $errors->has('id_data_layanan') ? $errors->first('id_data_layanan') : '' }}</span>
              </div>
            </div>
          @else
            {{ Form::hidden('id_data_layanan', $data->id_data_layanan) }}
          @endif
          <div class="form-group {{ $errors->has('syarat') ? 'has-error' : '' }}">
            {{ Form::label('Syarat Layanan','', ['class'=>'control-label']) }}
            <div class="col-sm-12">
              {{ Form::text('syarat',$data->syarat,['class'=>'form-control']) }}
