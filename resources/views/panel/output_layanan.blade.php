...
<div class="container-fluid">
    <div class="row mb-2">
        <div class="col-sm-6">
            <h1 class="m-0 text-dark">Output Layanan</h1>
        </div><!-- /.col -->
        <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
                <li class="breadcrumb-item"><a href="{{ url('dashboard') }}">Dashboard</a></li>
                <li class="breadcrumb-item active">Output Layanan</li>
            </ol>
        </div><!-- /.col -->
    </div><!-- /.row -->
</div><!-- /.container-fluid -->
  ...
  <div class="col-sm-12">
    ...
    <div class="card card-default">
      ...
      <div class="card-header text-right bg-teal">
        {{ Form::open(['url'=>'output_layanan', 'method'=>'post']) }}
        <div class="row">
          <div class="col-sm-4">
          ...
          <div class="col-sm-8">
            @if(isset($cari))
              <a href="{{ url('output_layanan') }}" class="btn btn-warning btn-sm"><i class="fa fa-refresh"></i> Reset</a>
            @endif
            @if(Helper::cek_level_akses('output_layanan')->t)
            <a href="{{ url('output_layanan/tambah') }}" class="btn btn-success btn-sm"><i class="fa fa-plus"></i> Tambah</a>
            @endif
          </div>
        </div>
        <!-- end row -->
            ...
            <tbody>
              ...
              <tr>
                ...
                <td class="text-center">
                  @if(Helper::cek_level_akses('output_layanan')->d)
                  <a href="{{url('output_layanan/detail')}}/{{Helper::encrypt($value->id)}}" class="btn btn-info btn-sm btn-detail"><i class="fa fa-fw fa-eye"></i></a>
                  @endif
                  @if(Helper::cek_level_akses('output_layanan')->u)
                  <a href="{{url('output_layanan/ubah')}}/{{Helper::encrypt($value->id)}}" class="btn btn-warning btn-sm btn-ubah"><i class="fa fa-fw fa-edit"></i></a>
                  @endif
                  @if(Helper::cek_level_akses('output_layanan')->h)
...
@section('js')
  <script type="text/javascript">
    function open_modal(id) {
      $('.modal-message').modal();
      $('#submit_hapus').attr("href","{{url('output_layanan')}}/hapus/"+id);
    }
  </script>
@stop
